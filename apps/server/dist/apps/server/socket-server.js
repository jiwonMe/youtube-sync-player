...
const roomsByName = new Map(); // Maps roomName (lowercase) to roomId
...
const httpServer = (0, http_1.createServer)((req, res) => {
    ...
    if (parsedUrl.pathname === "/rooms/by-name" && req.method === "GET") {
        const roomName = parsedUrl.query.name;
        if (!roomName) {
...
io.on("connection", (socket) => {
    ...
    socket.on("player:sync", (data) => {
        ...
        if (stateChanged || timeChanged) {
            ...
            socket.to(roomId).emit("player:sync", {
                ...
                syncTime: Date.now(),
                videoId: data.videoId || ((_a = room.currentVideo) === null || _a === void 0 ? void 0 : _a.videoId),
            });
    ...
    socket.on("autoplay:toggle", (data) => {
        ...
        if (user) {
            createEventLog(roomId, user, 'autoplayToggle', { autoplay: data.autoplay });
        }
    });
    // Handle video change
    socket.on("video:change", (videoId) => {
        ...
        if (video) {
            ...
            const user = room.users.find(u => u.id === userId);
            if (user) {
                createEventLog(roomId, user, 'videoChange', { videoId, videoTitle: video.title });
            }
        }
    });
    ...
    socket.on("playlist:update", (playlist) => {
        ...
        const user = room.users.find(u => u.id === userId);
        if (user) {
            createEventLog(roomId, user, 'playlistReorder');
        }
    });
    ...
    socket.on("playlist:add", (video) => {
        ...
        const user = room.users.find(u => u.id === userId);
        if (user) {
            createEventLog(roomId, user, 'playlistAdd', { videoId: video.videoId, videoTitle: video.title });
        }
    });
    ...
    socket.on("playlist:remove", (videoId) => {
        ...
        if (videoIndex !== -1) {
            ...
            const user = room.users.find(u => u.id === userId);
            if (user) {
                createEventLog(roomId, user, 'playlistRemove', { videoId, videoTitle: removedVideo.title });
            }
        }
    });
    // Handle chat message
    ...
    socket.on("room:update", (settings) => {
        ...
        if (settings.hostId && settings.hostId !== room.hostId) {
            ...
            if (userId === oldHostId) {
                ...
                if (newHost) {
                    // 이벤트 로그 생성
                    createEventLog(roomId, newHost, 'hostChange');
                }
                // 다른 모든 참가자들에게 전파
    ...
    socket.on("player:seek", (data) => {
        ...
        const user = room.users.find(u => u.id === userId);
        if (user) {
            createEventLog(roomId, user, 'seek', { time: data.currentTime });
        }
    });
    // 재생 이벤트 핸들러 추가
    ...
    socket.on("video:controlPermission", (permission) => {
        ...
        if (previousPermission !== permission) {
            ...
            const user = room.users.find(u => u.id === userId);
            if (user) {
                createEventLog(roomId, user, 'hostChange', // 기존 이벤트 타입 재사용 (또는 원하는 경우 새로운 이벤트 타입 추가)
                {
 ...
 */
function createEventLog(roomId, user, eventType, details) {
    const room = rooms.get(roomId);
    if (!room)
        return;
    // 로그 생성을 위한 사용자 정보 확인
    ...
    switch (eventType) {
        ...
        case 'pause':
            ...
            break;
        case 'seek':
            logMessage = `${user.name}님이 ${(details === null || details === void 0 ? void 0 : details.time) || 0}초로 영상을 이동했습니다.`;
            break;
        case 'videoChange':
            logMessage = `${user.name}님이 "${(details === null || details === void 0 ? void 0 : details.videoTitle) || ''}"(으)로 영상을 변경했습니다.`;
            break;
        default:
